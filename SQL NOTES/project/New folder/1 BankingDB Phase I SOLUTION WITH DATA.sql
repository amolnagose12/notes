...
-- STEP 2: CREATE DATABASE WITH RELEVANT FILES AND FILEGROUPS
...
-- REVISE THESE CONCEPTS:		JOIN TYPES + FUNCTIONS + QUERIES + GROUP BY + INDEXES + PIVOT  + JOIN OPTIONS
...
-- 11. LIST OF ALL BANKS WITH CUSTOMERS BUT NO TRANSACTIONS
-- 12. LIST OF ALL ZIP CODES WITH MISSING CUSTOMER ADDRESS
-- 13. MONTHLY STATEMENT transactions for a given month for a given customer id
-- 14. ACCOUNTS WITH DEPOSITS TXNs total deposits by month for any account in 2018
-- 15. ALL ACCOUNTS total monthly deposits even if deposits = 0
			please mail me your answers WITHOUT FAIL. IN REPLY TO YOUR ANSWERS, I SHALL MAIL YOU MY ANSWERS
...
A.accNumber, A.accTypeCode_fk, T.RunningBalance,T.tranTransactionAmount
...
/*
CURSORS		:	A SPECIAL TYPE OF VARIABLE, USED TO ACCESS ONE ROW AT A TIME.
PURPOSE		:	FOR ROW LEVEL OPERATIONS. EXAMPLE :  SPLIT ALL EVEN NUMBERED ROWS INTO ONE LIST, ODD ROWS TO ANOTHER
*/


SELECT
 C.cstId, C.cstFirstName + '   ' + C.cstLastName AS CUST_NAME, 
 A.accNumber, A.accTypeCode_fk, 
 ISNULL(T.RunningBalance,0) AS Run_Bal,  isnull(T.tranTransactionAmount, 0) as txn_Amt
FROM bank.tblcstCustomer AS C 
left outer JOIN
ACCOUNT.tblaccAccount AS A
on A.accCustomerId_fk=C.cstId 
...
SELECT
 C.cstId, C.cstFirstName + '   ' + C.cstLastName AS CUST_NAME, 
 A.accNumber, A.accTypeCode_fk, 
 convert(int, ISNULL(T.RunningBalance,0)) AS Run_Bal,  convert(int, isnull(T.tranTransactionAmount, 0)) as txn_Amt
FROM bank.tblcstCustomer AS C 
left outer JOIN
ACCOUNT.tblaccAccount AS A
on A.accCustomerId_fk=C.cstId 
